...
// Lógica compartida de importación masiva de tamaños desde Excel.
// Los tamaños se guardan como variantes de la categoría de filtro, así que
// funciona igual desde "editar producto" (que resuelve la categoría del
// producto) y desde "agregar producto" (que la toma del selector del form).
...
export interface ImportSizesResult {
  success: boolean;
  status: number;
  message: string;
...
export const importSizesForCategory = async (
  ...
  try {
    ...
    return { success: false, status: 400, message: 'El archivo no es un Excel válido' };
  }
