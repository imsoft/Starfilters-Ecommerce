...
const COLUMNAS_PRODUCTO: Array<{ nombre: string; ddl: string }> = [
  ...
  { nombre: 'nominal_size_en', ddl: 'VARCHAR(100) NULL' },
...
export const createProduct = async (productData: Partial<Product>): Promise<number | null> => {
  try {
    ...
    const optionalFields: { field: keyof Product; dbColumn: string }[] = [
      ...
      { field: 'product_code', dbColumn: 'product_code' },
      { field: 'air_flow', dbColumn: 'air_flow' },
      ...
      { field: 'nominal_size_en', dbColumn: 'nominal_size_en' },
      ...
      { field: 'initial_pressure_drop', dbColumn: 'initial_pressure_drop' },
      { field: 'recommended_final_pressure_drop', dbColumn: 'recommended_final_pressure_drop' },
  ...
  try {
    ...
    await query(
      `UPDATE products SET
        ...
        price = ?, currency = ?, price_usd = ?, nominal_size = ?, nominal_size_en = ?, real_size = ?,
        category = ?, category_en = ?, stock = ?,
        status = ?, tags = ?, dimensions = ?, weight = ?,
        material = ?, warranty = ?, bind_id = ?, bind_code = ?, sku = ?, product_code = ?, air_flow = ?,
      ...
      [
        ...
        productData.nominal_size_en || null,
        ...
        productData.initial_pressure_drop || null,
        productData.recommended_final_pressure_drop || null,
