...
import { deleteCaseStudy } from '@/lib/database';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const deleted = await deleteCaseStudy(uuid);
    if (!deleted) {
      return new Response(JSON.stringify({ success: false, message: 'Caso de éxito no encontrado' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
