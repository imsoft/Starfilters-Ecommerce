...
import { deletePortfolioProject } from '@/lib/database';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const deleted = await deletePortfolioProject(uuid);
    if (deleted) {
    ...
    return new Response(JSON.stringify({ success: false, message: 'No se encontró el proyecto' }), {
      status: 404,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
